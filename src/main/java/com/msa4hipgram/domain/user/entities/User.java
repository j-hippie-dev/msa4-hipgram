package com.msa4hipgram.domain.user.entities;
...
public class User {
    ...
    @Column(name = "password", nullable = false, length = 255) // length 안 적어주면 default값(255) 적용.
    ...
    @JdbcTypeCode(Types.VARCHAR) // Enum 객체 데이터 타입은 DB에서 알 수 없기 때문에 설정 해줘야 함
    ...
    private String refreshToken; // 안 주면 기본값 null

    @CreatedDate // 생성시 자동으로 시간 입력
    @Column(name = "created_at", nullable = false)
    private LocalDateTime createdAt;

    @LastModifiedDate // 수정시 자동으로 시간 업데이트
